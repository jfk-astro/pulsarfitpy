...
class PulsarFitGUI:
    ...
    def _create_parameter_section(self, parent):
        ...
        ttk.Label(param_frame, text="X Parameter:").grid(row=0, column=0, sticky=tk.W, pady=2)
        ...
        ttk.Label(param_frame, text="Y Parameter:").grid(row=1, column=0, sticky=tk.W, pady=2)
        ...
        ttk.Label(param_frame, text="Max Degree:").grid(row=2, column=0, sticky=tk.W, pady=2)
        self.degree_var = tk.IntVar(value=5)
        degree_spin = ttk.Spinbox(param_frame, from_=1, to=10, textvariable=self.degree_var, width=18)
        degree_spin.grid(row=2, column=1, sticky=(tk.W, tk.E), pady=2)
        
        self.log_x_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(param_frame, text="Log X", variable=self.log_x_var).grid(row=3, column=0, sticky=tk.W, pady=2)
        
        self.log_y_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(param_frame, text="Log Y", variable=self.log_y_var).grid(row=3, column=1, sticky=tk.W, pady=2)
        
        ttk.Label(param_frame, text="ATNF Condition:").grid(row=4, column=0, sticky=tk.W, pady=2)
        self.condition_var = tk.StringVar(value="")
        condition_entry = ttk.Entry(param_frame, textvariable=self.condition_var, width=20)
        condition_entry.grid(row=4, column=1, sticky=(tk.W, tk.E), pady=2)
        
        btn_frame = ttk.Frame(param_frame)
        btn_frame.grid(row=5, column=0, columnspan=2, pady=10)
        
        self.fit_btn = ttk.Button(btn_frame, text="Fit Polynomial", command=self._on_fit)
        self.fit_btn.grid(row=0, column=0, padx=5)
        
        self.query_btn = ttk.Button(btn_frame, text="Query ATNF", command=self._on_query)
    ...
    def _on_fit(self):
        def fit_thread():
            try:
                self.status_label.config(text="Fitting polynomial...", foreground="orange")
    ...
    def _plot_results(self):
        ...
        ax1.scatter(self.approx.x_data, self.approx.y_data, s=10, alpha=0.4, label='Data')
        ax1.plot(self.approx.predicted_x, self.approx.predicted_y, 'r-', linewidth=2,
                label=f'Degree {self.approx.best_degree}')
        xlabel = f"log({self.approx.x_param})" if self.approx.log_x else self.approx.x_param
        ylabel = f"log({self.approx.y_param})" if self.approx.log_y else self.approx.y_param
