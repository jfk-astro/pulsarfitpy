...
class PulsarPINN:
    ...
    def __init__(
        ...
        self.loss_log = {"total": [], "physics": [], "data": [], "val_total": [], "val_physics": [], "val_data": []}
    ...
    def _prepare_data(self):
        ...
        val_indices = shuffled_indices[num_train_points:num_train_points + num_val_points]
        ...
        for symbol, data_array in self.fixed_inputs.items():
            ...
            self.fixed_torch_train[variable_name] = self._numpy_array_to_tensor(data_array[train_indices])
            self.fixed_torch_val[variable_name] = self._numpy_array_to_tensor(data_array[val_indices])
            self.fixed_torch_test[variable_name] = self._numpy_array_to_tensor(data_array[test_indices])
    ...
    def _create_physics_residual(self):
        ...
        try:
            lambdify_expression = sp.lambdify(symbols, residual_expression, modules="torch")
        except Exception:
            lambdify_expression = sp.lambdify(symbols, residual_expression, modules="numpy")
    ...
    def _train_step(
        ...
        physics_residual = self.physics_residual_fn(self.x_train_torch, predicted_y, self.fixed_torch_train)
    ...
    def _validation_step(
        ...
        with torch.no_grad():
            ...
            physics_residual = self.physics_residual_fn(self.x_val_torch, predicted_y, self.fixed_torch_val)
            loss_physics = torch.mean(physics_residual ** 2)
    ...
    def _print_evaluation_report(self) -> None:
        ...
        for split_name in ["test", "val", "train"]:
            ...
            print(f"  Loss (Total):      {self.test_metrics[f'{split_name}_loss_total']:.6e}")
            print(f"  Loss (Physics):    {self.test_metrics[f'{split_name}_loss_physics']:.6e}")
            print(f"  Loss (Data):       {self.test_metrics[f'{split_name}_loss_data']:.6e}")
            ...
            if split_name == "test":
                print(f"  Reduced χ²:        {self.test_metrics[f'{split_name}_chi2_reduced']:.6f}")
        
        # Calculate difference between training and test R² scores
        train_test_r2_difference = self.test_metrics["train_r2"] - self.test_metrics["test_r2"]
    ...
    def predict_extended(
        ...
        with torch.no_grad():
            ...
            x_extended = torch.linspace(x_min - extend, x_max + extend, n_points, dtype=torch.float64).view(-1, 1)
    ...
    def plot_loss_curves(self, log_scale: bool = True) -> None:
        ...
        if self.loss_log["val_total"]:
            ...
            val_epochs = np.linspace(0, len(epochs)-1, len(self.loss_log["val_total"]))
            ax1.plot(val_epochs, self.loss_log["val_total"], 
        ...
        if self.loss_log["val_physics"]:
            ax2.plot(val_epochs, self.loss_log["val_physics"], 
                    label="Val Physics", linestyle=':', marker='s', markersize=3)
            ax2.plot(val_epochs, self.loss_log["val_data"], 
                    label="Val Data", linestyle=':', marker='s', markersize=3)
    ...
    def plot_predictions_vs_data(
        ...
        ax.scatter(self.x_train, self.y_train, c='blue', alpha=0.4, 
                s=30, label='Train Data', marker='o')
        
        # Plot validation data points (orange squares)
        ax.scatter(self.x_val, self.y_val, c='orange', alpha=0.5, 
                s=40, label='Validation Data', marker='s')
        
        # Plot test data points (red triangles)
        ax.scatter(self.x_test, self.y_test, c='red', alpha=0.6, 
                s=50, label='Test Data', marker='^')
        
        # Plot model prediction curve (green line, drawn on top with zorder=5)
        ax.plot(x_values, y_predictions, 'g-', linewidth=2.5, 
                label='PINN Prediction', zorder=5)
    ...
    def save_predictions_to_csv(
        ...
        metadata_lines = self._generate_metadata_lines(include_test_metrics, additional_metadata)
    ...
    def _generate_metadata_lines(
        ...
        lines.append(f"Total Parameters: {sum(p.numel() for p in self.model.parameters())}")
        ...
        if include_test_metrics and self.test_metrics:
            ...
            lines.append(f"  R² Score: {self.test_metrics.get('test_r2', float('nan')):.6f}")
            lines.append(f"  RMSE: {self.test_metrics.get('test_rmse', float('nan')):.6e}")
            lines.append(f"  MAE: {self.test_metrics.get('test_mae', float('nan')):.6e}")
            lines.append(f"  Reduced χ²: {self.test_metrics.get('test_chi2_reduced', float('nan')):.6f}")
    ...
    def _build_raw_data_dataframe(self, x_name: str, y_name: str) -> pd.DataFrame:
        ...
        for i in range(len(self.x_train)):
            data_rows.append({
                ...
                f"{y_name}_predicted": self.y_train[i],   # True output value (not predicted!)
    ...
    def set_learn_constants(self, new_constants: Dict[str, float]) -> None:
        ...
        for name, value in new_constants.items():
            if name in self.learnable_params:
                self.learnable_params[name].data = torch.tensor([value], dtype=torch.float64)
