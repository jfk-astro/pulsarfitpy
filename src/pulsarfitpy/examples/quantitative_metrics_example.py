...
# Query ATNF data
query = QueryATNF(
    params=["P0", "P1", "BSURF"], condition="exist(P0) && exist(P1) && exist(BSURF)"
)
table = query.table
...
ax1.scatter(
    ...
    alpha=0.4,
    c="blue",
    label=f"Train (n={len(pinn.x_train)})",
)
ax1.scatter(
    pinn.x_val,
    pinn.y_val,
...
ax1.scatter(
    ...
    alpha=0.5,
    c="green",
    label=f"Test (n={len(pinn.x_test)})",
)
...
if pinn.loss_log["val_total"]:
    ...
    val_epochs = list(range(0, len(pinn.loss_log["train_total"]), val_interval))[
        : len(pinn.loss_log["val_total"])
    ]
    ax2.plot(
        val_epochs, pinn.loss_log["val_total"], "o-", label="Val Total", linewidth=1.5
    )
...
query2 = QueryATNF(
    params=["P0", "P1"], condition="exist(P0) && exist(P1) && P0 > 0 && P1 > 0"
)

approx = PulsarApproximation(
    query=query2, x_param="P0", y_param="P1", test_degree=5, log_x=True, log_y=True
)
